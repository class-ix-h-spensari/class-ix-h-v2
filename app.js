...
/* ==========================================================================
   1. SISTEM MANAJEMEN UANG KAS KELAS IX-H
   - Tarif: Rp 1.000 / Hari (Senin - Sabtu = Rp 6.000 / Pekan)
...
// Default data mutasi jurnal kas
const defaultKasMutations = [
  { id: 'tx-1', type: 'masuk', title: 'Saldo Awal Kas Semester Genap IX-H', amount: 350000, category: 'Kas Awal', date: '2026-08-01' },
  { id: 'tx-2', type: 'keluar', title: 'Beli 3 Pcs Spidol Snowman Boardmarker & 1 Botol Tinta', amount: 35000, category: 'Alat Tulis', date: '2026-08-08' },
  { id: 'tx-3', type: 'keluar', title: 'Beli Sapu Ijuk Baru, Pengki & Kemoceng Bulu Ayam', amount: 48000, category: 'Kebersihan', date: '2026-08-14' },
  { id: 'tx-4', type: 'keluar', title: 'Sumbangan Kas & Menjenguk Teman Sakit', amount: 50000, category: 'Sosial', date: '2026-08-18' },
  { id: 'tx-5', type: 'masuk', title: 'Sisa Uang Hadiah Juara Lomba Kebersihan Kelas', amount: 100000, category: 'Lain-lain', date: '2026-08-20' },
  { id: 'tx-6', type: 'keluar', title: 'Beli Kain Pel Microfiber & Cairan Pembersih Lantai Wipol', amount: 26000, category: 'Kebersihan', date: '2026-08-22' }
];
...
function renderKasTable() {
  ...
  const filteredStudents = studentsData.filter(s => {
    const matchSearch = s.name.toLowerCase().includes(kasSearchQuery.toLowerCase()) || 
                        String(s.abs).includes(kasSearchQuery);
  ...
  filteredStudents.forEach(s => {
    ...
    const daysHtml = studentP.map((paid, dayIdx) => {
      ...
      return `<td style="text-align:center;">
        <div class="kas-day-check ${cls}" data-studentid="${s.id}" data-day="${dayIdx}" title="${paid ? 'Sudah Bayar Rp 1.000' : 'Belum Bayar'}">
          ${dayIcon}
        </div>
      </td>`;
    }).join('');
    ...
    html += `
      <tr>
        <td style="font-weight:700; color:var(--text-silver); text-align:center;">#${s.abs < 10 ? '0' + s.abs : s.abs}</td>
        <td>
  ...
  if (isBendaharaLoggedIn) {
    tbody.querySelectorAll('.kas-day-check').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (kasState.payments[studentId] && kasState.payments[studentId][currentKasWeek]) {
      ...
      <div class="kas-tx-item ${m.type}">
        <div class="kas-tx-left">
          ...
          <div class="kas-tx-info">
            <h5>${m.title}</h5>
            <p>
              <span><i class="fas fa-tag"></i> ${m.category || 'Umum'}</span>
              <span><i class="fas fa-calendar-day"></i> ${m.date || 'Terbaru'}</span>
            </p>
          </div>
        </div>
...
function generateWATagihan() {
  ...
  studentsData.forEach(s => {
    ...
    const daysPaid = p ? p.filter(Boolean).length : 0;
    if (daysPaid < 6) {
      unpaidStudents.push(`• ${s.abs}. ${s.name} (Kurang: ${formatRupiah((6 - daysPaid) * 1000)})`);
    }
  });
  ...
  if (unpaidStudents.length === 0) {
    text += `🎉 *Luar biasa! Seluruh siswa IX-H sudah LUNAS uang kas pekan ini!* Terima kasih atas ketertiban teman-teman semua. ✨`;
  } else {
  ...
  navigator.clipboard.writeText(text).then(() => {
    alert("✅ Format tagihan WhatsApp berhasil disalin ke clipboard! Silakan paste ke grup WA kelas IX-H.");
  }).catch(() => {
...
function exportKasToCSV() {
  let csv = "No,Nama Siswa,Senin,Selasa,Rabu,Kamis,Jumat,Sabtu,Total Bayar,Status\n";
  studentsData.forEach(s => {
    ...
    csv += `${s.abs},"${s.name}",${dayVals},${total},"${status}"\n`;
  });
  ...
  a.href = url;
  a.download = `Laporan_Kas_IX-H_${currentKasWeek}.csv`;
  a.click();
...
function getInitialAbsensiData() {
  ...
  months.forEach(m => {
    initialAbs[m] = {};
    studentsData.forEach((s, idx) => {
      // Data kerangka default 24 hari efektif sekolah per bulan
      let h = 24;
      let sCount = 0;
      let iCount = 0;
      let aCount = 0;
      ...
      initialAbs[m][s.id] = { h, s: sCount, i: iCount, a: aCount };
    });
  });
...
function renderAbsensiTable() {
  ...
  const filtered = studentsData.filter(s => {
    const matchSearch = s.name.toLowerCase().includes(absSearchQuery.toLowerCase()) || 
                        String(s.abs).includes(absSearchQuery);
    if (!matchSearch) return false;
...
function generateWAAbsensi() {
  ...
  studentsData.forEach(s => {
    ...
    if (rec.s > 0 || rec.i > 0 || rec.a > 0) {
      ...
      catatanSiswa.push(`• ${s.abs}. ${s.name} (${rincian.join(', ')})`);
    }
  });
  ...
  text += `_Laporan diarsipkan secara otomatis melalui Website Resmi Kelas IX-H._`;

  navigator.clipboard.writeText(text).then(() => {
    alert("✅ Format laporan absensi WhatsApp berhasil disalin ke clipboard! Silakan paste ke grup/chat Wali Kelas.");
  }).catch(() => {
...
function exportAbsensiCSV() {
  const monthData = absensiState[currentAbsMonth] || {};
  let csv = "No,Nama Siswa,Hadir (H),Sakit (S),Izin (I),Alpa (A),Total Hari,Persentase Kehadiran\n";

  studentsData.forEach(s => {
    const rec = monthData[s.id] || { h: 24, s: 0, i: 0, a: 0 };
    const totalDays = rec.h + rec.s + rec.i + rec.a;
    const pct = totalDays > 0 ? ((rec.h / totalDays) * 100).toFixed(1) : '100.0';
    csv += `${s.abs},"${s.name}",${rec.h},${rec.s},${rec.i},${rec.a},${totalDays},"${pct}%"\n`;
  });
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `Rekap_Absensi_IX-H_${currentAbsMonth}_2026.csv`;
  a.click();
  URL.revokeObjectURL(url);
}
...
function initKasAndAbsensi() {
  ...
  document.getElementById('btnToggleBendaharaModal')?.addEventListener('click', () => {
    if (isBendaharaLoggedIn) {
      alert("Anda sedang dalam mode bendahara aktif.");
    } else {
      if (pinModal) {
  ...
  pinForm?.addEventListener('submit', (e) => {
    e.preventDefault();
    if (pinInput.value.trim() === BENDAHARA_PIN) {
      isBendaharaLoggedIn = true;
      pinModal.style.display = 'none';
      if (alertBox) alertBox.style.display = 'flex';
      if (tableContainer) tableContainer.classList.add('editable-mode');
      document.getElementById('bendaharaBtnText').textContent = 'Bendahara Terbuka';
      renderKasTable();
    } else {
  ...
  document.getElementById('btnLogoutBendahara')?.addEventListener('click', () => {
    ...
    document.getElementById('bendaharaBtnText').textContent = 'Kelola Kas (PIN)';
    renderKasTable();
  });
  ...
  document.getElementById('btnOpenAddModal')?.addEventListener('click', () => {
    if (addTxModal) addTxModal.style.display = 'flex';
  });
  document.getElementById('closeAddTxModalBtn')?.addEventListener('click', () => {
    if (addTxModal) addTxModal.style.display = 'none';
  });
  ...
  if (absStudentSelect) {
    absStudentSelect.innerHTML = studentsData.map(s => `
      <option value="${s.id}">${s.abs}. ${s.name}</option>
    `).join('');
  }
  ...
  function syncModalValues() {
    ...
    if (inputH) inputH.value = rec.h;
    if (inputS) inputS.value = rec.s;
    if (inputI) inputI.value = rec.i;
    if (inputA) inputA.value = rec.a;
  }

  absStudentSelect?.addEventListener('change', syncModalValues);

  document.getElementById('btnOpenAbsensiEditModal')?.addEventListener('click', () => {
    if (absModal) {
      syncModalValues();
      absModal.style.display = 'flex';
    }
  });

  document.getElementById('closeAbsEditModalBtn')?.addEventListener('click', () => {
    if (absModal) absModal.style.display = 'none';
  });
  ...
  document.getElementById('absEditForm')?.addEventListener('submit', (e) => {
    ...
    absensiState[currentAbsMonth][sId] = {
      h: parseInt(inputH.value) || 0,
      s: parseInt(inputS.value) || 0,
      i: parseInt(inputI.value) || 0,
      a: parseInt(inputA.value) || 0
    };
    ...
    if (absModal) absModal.style.display = 'none';
    alert("✅ Data presensi siswa berhasil diperbarui!");
  });
}
